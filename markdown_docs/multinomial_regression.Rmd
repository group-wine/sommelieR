...
```{r}
red_f <- red %>% mutate(quality_f = factor(quality,levels = c("8", "3", "4","5", "6", "7")))

sat <- multinom(quality_f ~ 1 + alcohol, data = red_f, trace = 1, maxit = 300)
coef_sat <- t(coef(sat)) %>% matrix(nrow = 1)
